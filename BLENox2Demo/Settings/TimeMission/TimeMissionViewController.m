...
static NSString *const kRowEndTime = @"kRowEndTime";
static NSString *const kRowRepeat = @"kRowRepeat";
static NSString *const kRowOpenMode = @"kRowOpenMode";
static NSString *const kRowLight = @"kRowLight";
static NSString *const kRowMusic = @"kRowMusic";
static NSString *const kRowMusicVolumn = @"kRowMusicVolumn";


@interface TimeMissionViewController ()<UITableViewDataSource, UITableViewDelegate, SLPAlarmFooterDelegate>
...
@implementation TimeMissionViewController
...
- (void)setUI
{
    ...
    if (self.timeMissionPageType == TimeMissionPageType_Add) {
        self.titleLabel.text = LocalizedString(@"添加定时任务");
         self.addAlarmID = 1000 ;
        [self initDefaultAlarmData];
    }else{
        self.titleLabel.text = LocalizedString(@"编辑定时任务");
...
- (void)loadSectionData
{
    ...
    NSMutableArray *rowEnumList1 = [NSMutableArray arrayWithObjects:kRowTime, kRowEndTime, kRowRepeat, kRowOpenMode,kRowLight,kRowMusic, kRowMusicVolumn, nil];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    } else if([rowName isEqualToString:kRowEndTime]) {
        [self showTimeSelector];
    } else if ([rowName isEqualToString:kRowRepeat]){
        [self goSelectWeekdayPage];
    } else if ([rowName isEqualToString:kRowOpenMode]) {
        [self showOpenModeSelector];
    } else if ([rowName isEqualToString:kRowLight]) {
        [self goLightPage];
    }
...
- (UITableViewCell *)getTableCellWithRowName:(NSString *)rowName
{
    ...
    if ([rowName isEqualToString:kRowTime]) {
        ...
        baseCell.titleLabel.text = LocalizedString(@"开始时间");
        ...
        TitleSubTitleArrowCell *baseCell = (TitleSubTitleArrowCell *)[SLPUtils tableView:self.tableView cellNibName:@"TitleSubTitleArrowCell"];
        baseCell.titleLabel.text = LocalizedString(@"关闭时间");
        baseCell.subTitleLabel.text = [self getAlarmTimeStringWithDataModle:self.alarmDataNew];
        [Utils configCellTitleLabel:baseCell.textLabel];
        cell = baseCell;
    } else if ([rowName isEqualToString:kRowRepeat]){
        ...
        TitleSubTitleArrowCell *baseCell = (TitleSubTitleArrowCell *)[SLPUtils tableView:self.tableView cellNibName:@"TitleSubTitleArrowCell"];
        baseCell.titleLabel.text = LocalizedString(@"开启模式");
        baseCell.subTitleLabel.text = [SLPWeekDay getAlarmRepeatDayStringWithWeekDay:self.alarmDataNew.repeat];
        [Utils configCellTitleLabel:baseCell.textLabel];
        cell = baseCell;
    } else if ([rowName isEqualToString:kRowLight]) {
        TitleSubTitleArrowCell *baseCell = (TitleSubTitleArrowCell *)[SLPUtils tableView:self.tableView cellNibName:@"TitleSubTitleArrowCell"];
        baseCell.titleLabel.text = LocalizedString(@"灯光设置");
        baseCell.subTitleLabel.text = @"";
        [Utils configCellTitleLabel:baseCell.textLabel];
        cell = baseCell;
    } else if ([rowName isEqualToString:kRowMusic]){
        TitleSubTitleArrowCell *baseCell = (TitleSubTitleArrowCell *)[SLPUtils tableView:self.tableView cellNibName:@"TitleSubTitleArrowCell"];
        baseCell.titleLabel.text = LocalizedString(@"音乐设置");
        baseCell.subTitleLabel.text = [self getMusicNameWithMusicID:self.alarmDataNew.musicID];
        [Utils configCellTitleLabel:baseCell.textLabel];
        cell = baseCell;
    }else if([rowName isEqualToString:kRowMusicVolumn]){
        TitleSubTitleArrowCell *baseCell = (TitleSubTitleArrowCell *)[SLPUtils tableView:self.tableView cellNibName:@"TitleSubTitleArrowCell"];
        baseCell.titleLabel.text = LocalizedString(@"音量");
...
- (IBAction)saveAction:(UIButton *)sender {
    ...
    if (self.timeMissionPageType == TimeMissionPageType_Add) {
        [self goAddTimeMission];
       }else{
        [self editTimeMission];
       }
    __weak typeof(self) weakSelf = self;
